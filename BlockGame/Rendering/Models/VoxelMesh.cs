    ...
    public class VoxelMesh
    {
        ...
        public void FillRandom()
        {
            ...
            for (int x = 0; x < 16; x++)
                for (int y = 0; y < 16; y++)
                    for (int z = 0; z < 16; z++)
                        voxels[x, y, z] = (byte)rng.Next(0, 5);
        }
        ...
        public void Regenerate()
        {
            ...
            for (int x = 0; x < 16; x++)
            {
                for (int y = 0; y < 16; y++)
                {
                    for (int z = 0; z < 16; z++)
                    {
                        ...
                        AddSide(x, y, z, Vector3i.UnitX, voxel, neighbors);
                        AddSide(x, y, z, -Vector3i.UnitX, voxel, neighbors);
                        AddSide(x, y, z, Vector3i.UnitY, voxel, neighbors);
                        AddSide(x, y, z, -Vector3i.UnitY, voxel, neighbors);
                        AddSide(x, y, z, Vector3i.UnitZ, voxel, neighbors);
                        AddSide(x, y, z, -Vector3i.UnitZ, voxel, neighbors);
                    }
                }
            }
            ...
            MainThread.Run(() =>
            {
                mesh.Set(vertsArray, indArray, uvsArray, normalsArray);
            });
        }
        private void AddSide(int x, int y, int z, Vector3i dir, byte voxel, Chunk?[] neighbors)
        {
            int nx = x + dir.X;
            int ny = y + dir.Y;
            int nz = z + dir.Z;
        ...
        private void AddFace(int x, int y, int z, Vector3i normal, byte voxel)
        {
            ...
            AddTris(corners[0], corners[1], corners[2], uv[0], uv[1], uv[2], faceNormal);
            AddTris(corners[2], corners[3], corners[0], uv[2], uv[3], uv[0], faceNormal);
        }

        private void AddTris(Vector3 a, Vector3 b, Vector3 c, Vector2 uvA, Vector2 uvB, Vector2 uvC, Vector3 normal)
        {
